function [f_lm,freq,m_lm,module,w_lm,width] = gmodelall4_lls(temp,yoff,abside);
...
addpath /asl/matlab2012/srftest
...
if (length(d) ~= 2 | min(d) ~= 1 | max(d) ~= 1)
   error('argument temp must be dimensioned [1 x 1]')
end
d = size(yoff);
if (length(d) ~= 2 | min(d) ~= 1)
   error('argument yoff must be dimensioned [1 x 1] or [17 x 1]')
end
if (max(d) ~= 1 & max(d) ~= 17)
   error('argument yoff must be dimensioned [1 x 1] or [17 x 1]')
end
if (max(d) == 1)
   yoffx = yoff*ones(17,1);
else
   yoffx = yoff;
end
...
if (nargin == 3)
   ...
   elseif (abside == 'B')
      ...
      error('argument abside must be A or B')
   end
else
   load finalgm
end
...
for m=1:17
   y = yoffx(m) + y_cuton(m) + delta_t*yoffslope;
   ...
   if (abs(delta_t) > 1E-6 | abs(yoffx(m)) > 1E-6)
